...
int NsChannelFTD::scan(void)
{
    ...
    rc = FT_CreateDeviceInfoList((unsigned int*)&ndevs);
    if (rc != FT_OK)
    {
    ...
    if (ndevs > 0)
    {
        ...
        rc = FT_GetDeviceInfoList (dev, (unsigned int*)&ndevs);
        if (rc != FT_OK)
        {
...
int NsChannelFTD::readCommand(unsigned char *buf, size_t size)
{
    FT_STATUS rc;
    unsigned nbytes = 0;
    rc = FT_Read(ftdic, buf, size, (unsigned int*)&nbytes);
    if (rc != FT_OK)
    {
...
int NsChannelFTD::writeCommand(const unsigned char *buf, size_t size)
{
    FT_STATUS rc;
    unsigned nbytes = 0;
    rc = FT_Write(ftdic, (void *)buf, size, (unsigned int*)&nbytes);
...
int NsChannelFTD::readData(unsigned char *buf, size_t size)
{
    FT_STATUS rc2;
    unsigned nbytes = 0;
    rc2 = FT_Read(ftdid, buf, size, (unsigned int*)&nbytes);
    if (rc2 != FT_OK)
    {
